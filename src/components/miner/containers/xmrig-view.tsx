    ...
    const RenderHashrateChartVictory = React.useCallback(() => (
        <View style={{left: 0, bottom: 0, width: fullWidth}}>
            <Shimmer opacity={0.8} tilt={30} direction="left" pauseDuration={2500}>
              <VictoryArea  width={fullWidth}  padding={0} height={70} data={hashrateHistory} style={{data: { fill: 'rgba(134, 65, 244)'}}} interpolation="natural" standalone={true} />
            </Shimmer>
        </View>
        ...
        <>
            ...
            <View style={styles.row}>
                <MinerCard 
                    ...
                    disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{formatHashrate(minerData?.hashrate.total[0])[0]}</Paragraph>
                </MinerCard>
                <MinerCard
                    title="60s"
                    ...
                    style={{ flex: 2, marginRight: 10 }}
                    disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{formatHashrate(minerData?.hashrate.total[1])[0]}</Paragraph>
                </MinerCard>
                <MinerCard
                    title="15m"
                    ...
                    style={{ flex: 2, marginRight: 10 }}
                    disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{formatHashrate(minerData?.hashrate.total[2])[0]}</Paragraph>
                </MinerCard>
                <MinerCard
                    title="max"
                    ...
                    style={{ flex: 2 }}
                    disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{formatHashrate(minerData?.hashrate.highest)[0]}</Paragraph>
                </MinerCard>
            </View>
            <View style={styles.row}>
            ...
            <View style={styles.row}>
                <MinerCard title="Accepted" style={{ flex: 2, marginRight: 10 }} disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{minerData?.results.shares_good}</Paragraph>
                </MinerCard>
                <MinerCard title="Difficulty" style={{ flex: 2 }} disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{minerData?.results.diff_current}</Paragraph>
                </MinerCard>
            </View>
            <View style={styles.row}>
                <MinerCard title="Threads" style={{ flex: 2, marginRight: 10 }} disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{minerData?.cpu.threads}</Paragraph>
                </MinerCard>
                <MinerCard title="Avg Time" style={{ flex: 2 }} disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{minerData?.results.avg_time}</Paragraph>
                </MinerCard>
            </View>
            <View style={[styles.row]}>
                <MinerCard title="Free Mem" style={{ flex: 2, marginRight: 10 }} disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{minerData?.resources.memory.free}</Paragraph>
                </MinerCard>
                <MinerCard title="Res. Mem" style={{ flex: 2 }} disabled={disabled}>
                    <Paragraph adjustsFontSizeToFit numberOfLines={1}>{minerData?.resources.memory.resident_set_memory}</Paragraph>
                </MinerCard>
            </View>
        </>
    )
